...
class Generator_Encoder_Decoder:
    ...
    def __call__(self, inputs, y, nums_class, num_channel=3):
        with tf.variable_scope(name_or_scope=self.name, reuse=tf.AUTO_REUSE):
            ...
            embedding = conv("Gconv1", inputs, k_size=1, nums_out=8, strides=1)  # [n, 64, 64, 8]
...
class Discriminator_Ordinal:
    ...
    def __call__(self, inputs, y, nums_class, update_collection=None):
        with tf.variable_scope(name_or_scope=self.name, reuse=tf.AUTO_REUSE):
            ...
            inputs = conv("Dconv1", inputs, k_size=1, nums_out=8, strides=1)  # [n, 64, 64, 8]
            print(inputs)
            inputs = global_sum_pooling(inputs)  # [n, 8]
...
#             print(':', inputs)
#             embedding = G_Resblock_Encoder("Encoder-ResBlock1", inputs, 512, y,
#                                         nums_class)  # [n, 32, 32, 512]
#             print(':', embedding)
#
#             inputs = G_Resblock("ResBlock1", embedding, 256, y, nums_class)  # [n, 64, 64, 256]
#             print(':', inputs)
