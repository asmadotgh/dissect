    ...
    for e in range(1, EPOCHS + 1):
        ...
        for i in range(data.shape[0] // BATCH_SIZE):
            ...
            _, d_loss, summary_str = sess.run([D_opt, D_loss, d_sum],
                                              feed_dict={y_t: target_labels, x_source: img, train_phase: True,
                                                         y_s: labels})
            writer.add_summary(summary_str, counter)

            if (i + 1) % 5 == 0:
                _, g_loss, summary_str = sess.run([G_opt, G_loss, g_sum],
                                                  feed_dict={y_t: target_labels, x_source: img, train_phase: True,
                                                             y_s: labels})
                writer.add_summary(summary_str, counter)
            ...
            def save_results(sess, step):
                ...
                FAKE_IMG, fake_logits_ = sess.run([fake_target_img, fake_target_logits],
                                                  feed_dict={y_t: target_labels, x_source: img_repeat,
                                                             train_phase: False})
